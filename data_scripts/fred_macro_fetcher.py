...
def fetch_and_save_fred_data(series_id: str, start_date: datetime.datetime, end_date: datetime.datetime):
...
@click.command()
@click.option('--series', '-s', default=DEFAULT_SERIES, help="List of FRED identifiers (comma-separated)")
@click.option('--years', '-y', default=10, type=int, help="Number of years back to fetch")
def main(series: str, years: int):
    ...
    click.secho(f"=== Fetching macro data from the last {years} years ===", fg="cyan", bold=True)
    click.secho(f"Period: {start_date.strftime('%Y-%m-%d')} to {end_date.strftime('%Y-%m-%d')}\n", fg="cyan")
