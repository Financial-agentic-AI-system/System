...
@click.option(
    '--interval',
    '-i',
    type=click.Choice(['1m', '5m', '15m', '30m', '60m', '1d', '1wk', '1mo'], case_sensitive=False),
    default='1d',
...
@click.option(
    ...
    help="Output CSV filename in datalake"
)
def main(tickers: str, start: datetime.datetime, end: datetime.datetime, interval: str, output: str):
    """
    ...
    click.secho(f"Period: {start.strftime('%Y-%m-%d')} to {end.strftime('%Y-%m-%d')}", fg="cyan")
